...
import {
  ...
  torusPolygons,
  wireframeStrokes,
} from "../../src/domain/solids.js";

const PHI = (1 + Math.sqrt(5)) / 2;

describe("domain/solids - polyhedra", () => {
  it("tetrahedron has 4 vertices and 6 edges", () => {
    const mesh = tetrahedron();
    assert.equal(mesh.vertices.length, 4);
    assert.equal(mesh.edges.length, 6);
  });

  it("cube has 8 vertices and 12 edges", () => {
    const mesh = cube();
    assert.equal(mesh.vertices.length, 8);
    assert.equal(mesh.edges.length, 12);
  });

  it("octahedron has 6 vertices and 12 edges", () => {
    const mesh = octahedron();
    assert.equal(mesh.vertices.length, 6);
    assert.equal(mesh.edges.length, 12);
  });

  it("icosahedron has 12 vertices and 30 edges of length 2", () => {
  ...
  it("dodecahedron has 20 vertices and 30 edges of length 2/PHI", () => {
  ...
  it("greatIcosahedron shares the icosahedron wireframe (12/30)", () => {
    const mesh = greatIcosahedron();
    assert.equal(mesh.vertices.length, 12);
    assert.equal(mesh.edges.length, 30);
  });

  it("greatIcosahedronFaces yields 20 closed star faces over icosahedron edges", () => {
  ...
  it("starOctangula has 8 vertices and 12 edges (two tetrahedra)", () => {
    const mesh = starOctangula();
    assert.equal(mesh.vertices.length, 8);
    assert.equal(mesh.edges.length, 12);
  });

  it("tesseract has 16 vertices and 32 edges (4D hypercube)", () => {
    const mesh = tesseract();
    assert.equal(mesh.vertices.length, 16);
    assert.equal(mesh.edges.length, 32);
  });

  it("solidMesh dispatches every solid name", () => {
    for (const name of [
      "tetrahedron",
      "cube",
      "octahedron",
...
describe("domain/solids - projection", () => {
  it("rotate3D rotates 90 degrees around X: (0,1,0) -> (0,0,1)", () => {
  ...
  it("projectMesh renders one 2-point stroke per edge", () => {
  ...
  it("perspective projection magnifies points near the camera", () => {
...
describe("domain/solids - generated meshes", () => {
  it("torusPolygons yields segments + rings closed polygons", () => {
  ...
  it("torusKnotPoints is a closed curve with steps + 1 points", () => {
  ...
  it("revolutionPolygons yields profile rings + meridians", () => {
  ...
  it("wireframeStrokes renders explicit edges as 2-point strokes", () => {
