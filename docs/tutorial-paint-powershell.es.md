...
`scripts/paint-uia-bridge.ps1` y `src/infrastructure/win32/` de este
repositorio.
...
El driver del repo inyecta el gesto **íntegro por `SendInput`**: posición
inicial absoluta, botón abajo, movimientos absolutos y botón arriba
(`dragPolyline` en `src/infrastructure/win32/process.ts`). Mezclar
`SetCursorPos` con `SendInput` rompe el trazo en Paint moderno (el mouse-down
no queda sincronizado con la posición), así que aquí usamos `SendInput` para
todo:
...
> delay configurable, dwells de 60/90 ms antes del trazo y la conversión
> lienzo→cliente según el tamaño lógico.
...
function Invoke-PaintDrag {
  param([IntPtr]$Hwnd, [int[][]]$Points, [int]$StepMs = 10)
  # …SendInput: posición absoluta + LEFTDOWN/LEFTUP (sección 6)…
}
```
...
5. **Atajos de herramienta clásicos (B/P/E) como tecla suelta ya no existen**
   — pero las **KeyTips de la cinta sí funcionan**: pulsa y suelta `Alt`,
   espera ~250 ms y luego la letra (p. ej. `B` para la Brocha, que vive en un
   split-button cuyo InvokePattern por UIA solo abre el flyout). El driver
   del repo las usa (`pressKeyTip` en `src/infrastructure/win32/process.ts`),
   y son independientes del idioma y de la posición de la ventana.
...
9. **Dibujo = mouse (SendInput), no UIA** — UIA sirve para leer estado y
   operar controles; para trazos usa `SendInput` con posición absoluta y
   eventos de botón (todo el gesto por el mismo mecanismo).
